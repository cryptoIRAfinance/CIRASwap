...
const TransactionHistory = ({ data }: { data: DataType }) => {
  ...
  const activeTabColor = useColorModeValue('brand.500', 'brand.50');
  const nonActiveTabColor = useColorModeValue('brand.500', 'brand.50');
  const borderColor = useColorModeValue('brand.400', 'brand.400');
  const successColor = useColorModeValue('lightbg.700', 'lightbg.600');
  const bgColor = useColorModeValue("brand.200", "brand.400");
  const textColor = useColorModeValue("brand.50", "gray.7000");
      ...
      >
        ...
        <Grid templateColumns="repeat(3, 2fr)" gap={7} py={2}>
          <Box>
            ...
            <Text color={textColor} fontSize="14px" fontWeight="regular">
              Straight Swap
            </Text>
          </Box>
          <Box>
