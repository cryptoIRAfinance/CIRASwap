...
const FindPool = () => {
  ...
  const infoBg = useColorModeValue('brand.50', 'brand.50');
  const genBorder = useColorModeValue('brand.100', 'brand.500');
  const bgColor = useColorModeValue('brand.100', 'brand.700');
  const topIcons = useColorModeValue('gray.600', 'brand.50');
  const textColorOne = useColorModeValue('gray.700', 'brand.50');
  const manageColor = useColorModeValue('brand.500', 'tan.500');
  const hrBorderColor = useColorModeValue('brand.300', 'brand.500');
  ...
  return (
    <Center m={8}>
      ...
      >
        ...
        <Box
          bg={mode === 'dark' ? 'brand.700' : 'brand.100'}
          borderRadius="md"
          p={4}
          mt={4}
          mb={5}
        ...
        {loading ? (
          <Flex
            ...
            alignItems="center"
            backgroundColor={mode === 'dark' ? 'brand.700' : 'brand.100'}
            border={mode === 'dark' ? '1px solid brand.500' : '1px solid brand.100'}
            borderRadius="6px"
          >
            <Text fontSize="sm" color={mode === 'dark' ? 'brand.600' : 'gray.600'}>
              Loading...
            </Text>
          </Flex>
        ) : !account ? (
