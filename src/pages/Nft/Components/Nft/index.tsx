...
export const Nft = function ({ nftName, image, number, id, priceUSD, priceRGP, isFeatured = false }: NftProps) {
  ...
  return (
         <Box
          bg={useColorModeValue('tan.500', 'brand.500')}
          maxW="sm"
          borderWidth="1px"
          rounded="lg"
          ...
          >
            <Flex p={2.5} height={'320px'} justifyContent={'center'} alignItems={'center'}>
                {loading ? <Spinner speed="0.65s" color="gray.700" /> :  chainId === Number(SupportedChainId.BINANCETEST) || chainId === Number(SupportedChainId.POLYGONTEST) ? (
                    <Image
                        src={nftImage}
                        alt={`Picture`}
                        roundedTop="lg"
          ...
          <Box p="3">
            ...
            <Flex mt="2" justifyContent="space-between" alignContent="center">
              <Text/>
              <Text textColor={lightTextColor} >≈ {rgpPrice} CIRA</Text>
            </Flex>
